...
async def upload_excel(file: UploadFile = File(...)) -> UploadResponse:
    ...
    try:
        columns, sample_rows, row_count = read_excel_preview(destination, settings.sample_row_count)
    ...
    logger.info("Upload stored upload_id={} rows={} columns={}", upload_id, row_count, len(columns))
